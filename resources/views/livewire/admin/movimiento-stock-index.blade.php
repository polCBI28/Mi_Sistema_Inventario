...
<div class="w-full py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800" style="position: relative; background: linear-gradient(135deg, #18181b 0%, #27272a 100%); backdrop-filter: blur(10px);">
        ...
        <form action="{{ route('admin.movimiento_stock.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Producto -->
            <div style="animation: fade-in-up 0.3s ease-out 0.1s both;">
                <label for="producto_id" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.2s both;">
                <label for="tipo_movimiento" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.3s both;">
                <label for="cantidad" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.4s both;">
                <label for="stock_anterior" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
                    Stock Anterior <span class="text-red-500">*</span>
                </label>
                <input type="number" name="stock_anterior" id="stock_anterior" min="0" value="{{ old('stock_anterior') }}"
                    class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    style="transition: all 0.3s ease; background: rgba(39, 39, 42, 0.5);">
                @error('stock_anterior')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.5s both;">
                <label for="stock_actual" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
                    Stock Actual <span class="text-red-500">*</span>
                </label>
                <input type="number" name="stock_actual" id="stock_actual" min="0" value="{{ old('stock_actual') }}"
                    class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    style="transition: all 0.3s ease; background: rgba(39, 39, 42, 0.5);">
                @error('stock_actual')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.6s both;">
                <label for="motivo" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
                    Motivo
                </label>
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.7s both;">
                <label for="fecha_movimiento" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
            ...
            <div class="relative my-8" style="animation: fade-in-up 0.3s ease-out 0.8s both;">
                <div class="absolute inset-0 flex items-center">
                    <div class="w-full divider-gradient"></div>
                </div>
            </div>

            <div class="text-sm text-zinc-500 mb-6" style="animation: fade-in-up 0.3s ease-out 0.9s both; padding: 12px; background: rgba(59, 130, 246, 0.05); border-left: 3px solid #3b82f6; border-radius: 4px;">
                Campos marcados con <span class="text-red-500 font-bold">*</span> son obligatorios
            </div>

            <div class="flex justify-end" style="animation: fade-in-up 0.3s ease-out 1s both;">
                <button type="submit"
