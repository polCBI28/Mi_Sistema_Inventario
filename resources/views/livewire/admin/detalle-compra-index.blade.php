...
<div class="w-full py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800" style="position: relative; background: linear-gradient(135deg, #18181b 0%, #27272a 100%); backdrop-filter: blur(10px);">
        ...
        <form action="{{ route('admin.detallecompra.store') }}" method="POST" class="space-y-6">
            @csrf

            {{-- Campo compra_id --}}
            <div style="animation: fade-in-up 0.3s ease-out 0.1s both;">
                <label for="compra_id" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
                    ID de la Compra <span class="text-red-500">*</span>
                </label>
                <input type="number" id="compra_id" name="compra_id" min="1"
                    class="w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg 
                    focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 text-white"
            ...
            <div style="animation: fade-in-up 0.3s ease-out 0.4s both;">
                <label for="precio_unitario" class="block text-sm font-medium text-zinc-300 mb-1 field-label">
            ...
            {{-- Separador visual --}}
            <div class="relative my-8" style="animation: fade-in-up 0.3s ease-out 0.5s both;">
                <div class="absolute inset-0 flex items-center">
                    <div class="w-full divider-gradient"></div>
                </div>
            </div>

            <div class="text-sm text-zinc-500 mb-6" style="animation: fade-in-up 0.3s ease-out 0.6s both; padding: 12px; background: rgba(59, 130, 246, 0.05); border-left: 3px solid #3b82f6; border-radius: 4px;">
                Campos marcados con <span class="text-red-500 font-bold">*</span> son obligatorios
            </div>

            {{-- Botón --}}
            <div class="flex justify-end" style="animation: fade-in-up 0.3s ease-out 0.7s both;">
                <button type="submit"
                    class="px-6 py-3 bg-blue-600 text-white font-medium rounded-lg 
                    hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 
                    focus:ring-offset-2 focus:ring-offset-zinc-900 transition-all duration-200 shadow-lg hover:shadow-xl"
